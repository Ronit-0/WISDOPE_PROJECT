...
with tab6:
    ...
    if not st.session_state.logged_in:
            ...
            if submit_button:
                if login_email and login_pass:
                    if login_email == st.secrets["admin"]["email"] and login_pass == st.secrets["admin"]["password"]:
                        ...
                        st.session_state.force_tab_6 = True # THE FIX: Triggers Tab Jump
                        st.rerun()
                    else:
                        try:
                            from streamlit_gsheets import GSheetsConnection
        ...
        if st.button("Log Out"):
            st.session_state.logged_in = False
            st.session_state.force_tab_6 = True # THE FIX: Triggers Tab Jump back to login
            st.rerun()
...
# ==========================================
#       THE TAB MAGNET (GLOBAL FIX)
# ==========================================
# This script must sit at the absolute bottom of the file so it can see all the tabs!
if st.session_state.get("force_tab_6", False):
    import streamlit.components.v1 as components
    components.html(
        """
        <script>
    ...
    # Turn the magnet off so it doesn't force you into Tab 6 later when you don't want it to
    st.session_state.force_tab_6 = False
